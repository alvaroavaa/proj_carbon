...
# Carrega coordenadas do arquivo KML
def carregar_coordenadas_kml(kml_path):
    try:
        with open(kml_path, 'r', encoding='utf-8') as file:
            raw_content = file.read()

        # Remover possíveis atributos problemáticos
        raw_content = raw_content.replace('xsi:schemaLocation="...', '')
    ...
    for _, row in df.iterrows():
        ...
        if coordenadas_list:
            for coordenadas in coordenadas_list:
                if coordenadas:  # Verifica se não está vazio
                    folium.Polygon(
